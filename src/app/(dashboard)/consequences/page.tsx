...
export default function ConsequencesPage() {
  ...
  const warningsCount = myConsequences.filter((c) => c.type === 'INFORMAL_SANCTION').length;
  const infractionsCount = myConsequences.filter((c) => c.type === 'INFRACTION').length;
  const suspensionsCount = myConsequences.filter((c) => c.type === 'SUSPENSION').length;
    ...
    <div className="space-y-6 max-w-6xl mx-auto">
      ...
      <div>
        ...
        <p className="text-slate-500 text-xs mt-1 font-medium">
          View all disciplinary actions issued to you. Realtime WebSocket sync active.
        </p>
      </div>
